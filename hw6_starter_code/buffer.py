...
class Buffer:
    ...
    def reset(self):
        ...
        self.obs = torch.zeros(
            (self.capacity, self.obs_dim), dtype=torch.float32, device=self.device
        )
        self.next_obs = torch.zeros(
            (self.capacity, self.obs_dim), dtype=torch.float32, device=self.device
        )
        self.actions = torch.zeros(
            (self.capacity, self.act_dim), dtype=torch.float32, device=self.device
        )
        ...
        if filter is not None:
            ...
            for key, val_list in filter.items():
                ...
                val_tensor = torch.as_tensor(
                    val_list, device=self.device, dtype=attr.dtype
                )
