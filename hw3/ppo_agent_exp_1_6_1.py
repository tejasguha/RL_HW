...
class PPOAgent:
    ...
    def _perform_update(self) -> Dict[str, float]:
        ...
        for _ in range(self.update_epochs):
            for _ in range(n_updates_per_epoch):
                minibatch = self._rollout_buffer.sample(self.minibatch_size, {"iteration": list(range(1, self._policy_iteration+1))}) # sample from entire RB
                minibatch['advantages'] = (minibatch['advantages'] - minibatch['advantages'].mean())/(minibatch['advantages'].std() + 1e-8)
                loss, stats = self._ppo_loss(minibatch)
                all_stats.append(stats)
        ...
        for stat in all_stats:
            d_curr += stat['kl']
            length += 1
        d_curr = d_curr / length
        if d_curr > self.target_kl*1.5:
